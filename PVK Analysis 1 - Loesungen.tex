...
\begin{document}
\title{PVK Analysis - Lösungen}
\author{MB \& LB, Vorlage gemäss SeSc}
\maketitle

Die Verweise auf Theoreme und Propositionen beziehen sich auf das Skript der Vorlesung, welches Sie $\href{http://www.math.uzh.ch/index.php?ve_vo_det&key2=2513&keySemId=31}{hier}$ finden k\"onnen. Die Verweise auf Serien beziehen sich auf das HS 15.
...
\end{claim}
\begin{proof}
Wir definieren $h(x)= 2x+1-\log(2x+1)$. 

Dann gilt $h(0)=1$ und $h'(x)= 2 - \frac{1}{2x+1} >0$.
\end{proof}
Damit erhalten wir:
...
Aus dem Qutientenkriterium (Satz 4.12) folgt, dass $\sum_{n\geq 0} \frac{\exp(3n \log(n))}{(n!)^2}$ nicht absolut konvergiert. Da alle Terme nichtnegativ sind (dann ist absolute Konvergenz und Konvergenz das gleiche), folgt, dass $\sum_{n\geq 0} \frac{\exp(3n \log(n))}{(k!)^2}$ nicht konvergiert.
\end{enumerate}
\end{solution}
