...
Die Verweise auf Theoreme und Propositionen beziehen sich auf das Skript der Vorlesung, welches Sie $\href{http://www.math.uzh.ch/index.php?ve_vo_det&key2=2513&keySemId=31}{hier}$ finden k\"onnen. Die Verweise auf Serien beziehen sich auf das HS 15. Es handelt sich bei dieser Übungssammlung um eine überarbeitete Version der von SeSc zur Verfügung gestellten Vorlage. 
...
\end{enumerate}
\end{solution}
\begin{ex} Sei $(z_n)_{n\in \NN} \subseteq \CC$, $z\in \CC$. Entscheiden Sie f\"ur jede Aussage, ob sie wahr ist oder falsch.
...
\begin{enumerate}[i)]
\item $f:(0, \infty) \to \mathbb{R}, \ x \mapsto \frac{1}{1+x^2}$
\item $f:[0,1) \to \mathbb{R}, \ x \mapsto \sqrt{x}$ 
...
\item Finden Sie eine Funktion, die Lipschitz-stetig, aber nicht diferenzierbar ist.
...
\item Für die Ableitung an der Stelle $x=0$ gilt \begin{align*}
\lim_{x \to 0} |f'(x)| = \lim_{x \to 0} \left| \frac{1}{2 \sqrt{x}} \right| = \infty
\end{align*}
...
\item Wir definieren $f: [0,1] \to \mathbb{R}, \ x \mapsto \sqrt{x}$, dann ist $f$ gleichmässig stetig, aber nicht Lipschitz.
\item Die Betragsfunktion $f: \mathbb{R} \to \mathbb{R}, \ x \mapsto |x|$ ist ein möglicher Kandidat. 
\end{enumerate}
...
\end{ex}
\begin{solution}
Wir definieren $a:=\inf_{x\in \RR} f(x)$ und $b:=\sup_{x\in \RR} f(x)$. Dann gilt $Im(f) \subseteq [a,b]$ und wir k\"onnen $f$ einschr\"anken auf $f:[a,b] \rightarrow [a,b]$. Da $f$ stetig ist, folgt die Behauptung dem Fixpunktsatz.
\end{solution}
...
\begin{ex} Berechnen Sie den folgenden Grenzwert mit Hilfe der Lagran'schen Fehlerabschätzung $$ \lim_{x \rightarrow 0} \bigg( \frac{1}{e^x-1} - \frac{1}{x} \bigg).$$
\end{ex}
...
\begin{ex}
Beweisen Sie die verallgemeinerte Bernoulli-Ungleichung, diese besagt, dass f\"ur alle $r\geq 1, \ x\geq 0$ gilt:
$$ (1+x)^r\geq 1+rx .$$
...
\begin{ex} Sei im folgenden $f:[a,b] \to \mathbb{R}$ stetig und auf $(a,b)$ differenzierbar. 
\begin{enumerate}[i)]
