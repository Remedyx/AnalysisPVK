...
%Erste Aufgabe wird es sein, die Übungen zu sortieren! So wie ich die Aufgaben überflogen habe, sind nicht alle in einer Reihenfolge.
%Zweite Aufgabe ist es, alle Aufgaben zu metrischen/normierten Räumen sowie analytischen Funktionen wegzulassen!
%Abgrall hat diese Themen gar nicht behandelt!
%Ausserdem ist anscheinend Aufgabe 46 falsch!
%Die Sortierung sollte (bis jetzt) folgendermassen aussehen:
%1. Induktion
%2. sup, inf, max, min von Mengen
%3. Folgen
%4. Reihen
%5. Stetigkeit
...
\begin{document}
\title{L\"osungen PVK Analysis}
\author{MB \& LB, Vorlage gemäss SeSc}
\maketitle

Die Verweise auf Theoreme und Propositionen beziehen sich auf das Skript der Vorlesung, welches Sie $\href{http://www.math.uzh.ch/index.php?ve_vo_det&key2=2513&keySemId=31}{hier}$ finden k\"onnen. Die Verweise auf Serien beziehen sich auf das HS 15. Es handelt sich bei dieser Übungssammlung um eine Überarbeite Version der von SeSc zur Verfügung gestellten Vorlage. 
...
$$ (1+x)^0=1 = 1+ 0\cdot x. $$
Sei $n\in \NN_{\geq 1}$ beliebig, aber fix, sodass 
$(1+x)^{n} \geq 1+nx$ (IA). 

Dann berechnen wir
$$ (1+x)^{n+1} = (1+x)(1+x)^n \stackrel{\text{IA, }  x>-1}{\geq} (1+x)(1+nx)= 1+(n+1)x+nx^2 \geq 1 + (n+1)x.$$ 
\item
Zuerst zeigen wir den Fall $n=2$. Seien $x_1, x_2 \geq 0$.
...
\begin{ex} \
 \begin{enumerate}[i)]
 \item Zeigen Sie, dass der kleinste, nicht triviale Teiler einer natürlichen Zahlen $n \in \mathbb{N}_{ \geq 2}$ stets eine Primzahl ist. Hier ist keine vollständige Induktion notwendig, ein Beweis per Widerspruch genügt völlig. 
...
&= \frac{2 \cos \theta \sin((k+1)\theta)- \cos \theta \sin((k+)\theta) + \cos((k+1)\theta) \sin \theta}{\sin \theta} \\
&= \frac{\cos \theta \sin ((k+1)\theta)+ \cos((k+1)\theta) \sin \theta}{\sin \theta} \\
&= \frac{\sin((k+2) \theta}{\sin \theta }
\end{align*}
\end{solution}

\begin{ex} (*) Sei $n \in \NN$ und $a_{jk} \in \mathbb{C}$ für $j = 0,\dots,n + 1$, $k = 0,\dots,n$. Zeigen Sie, dass
	\begin{equation*}
...
\begin{align*}
S:= \left\lbrace \frac{n}{n+1} : n \in \mathbb{N}_0 \right\rbrace = \left\lbrace \frac{1}{2}, \frac{2}{3}, \frac{3}{4}, \dots \right \rbrace 
\end{align*}
...
\end{enumerate}
Wir stellen nun fest, dass jedes Element $s \in S$ kleiner als $1$ ist,  da $\frac{n}{n+1} <1$. Wir behauptet,  dass $1$ das Supremum der Menge ist. Nehme zum Widerspruch an, dass $1$ nicht die kleinste obere Schranke von $S$ ist, d.h. es existiert $\epsilon >0$ sodass $1- \epsilon$ auch eine obere Schranke von $S$ ist. 
\\\\
Wähle $n_0 \in \mathbb{N}$ sodass $n > \frac{1}{\epsilon}-1$, dann gilt $1- \epsilon < \frac{n_0}{n_0+1}$ was zeigt, dass $1- \epsilon$ keine obere Schranke von $S$ ist, da $\epsilon >0$ beliebig war, muss $1$ die kleinste obere Schranke von $S$ sein.
\\\\
...
\begin{enumerate}[i.)]
\item
$A$ unbeschr\"ankt $\Leftrightarrow$ $\sup A = \infty$.

\item
$A$ ist endlich $\Rightarrow$ $max A = sup A$.

\item
$min A$ existiert $\Rightarrow$ $min A = -max(-A)$.
...
\begin{solution} Der kleinste Term scheint $\frac{3}{2}$ zu sein und es scheint kein grösstes Element zu geben, aber alle Elemente sind kleiner als $2$ und da $\lim \frac{2n+1}{n+1}=2$ behaupten wir folgende Aussagen: \\
...
\\\\
Wir nehmen also an, dass $1$ nicht die grösste untere Schranke von $S$ ist, dann existiere ein $\epsilon >0$ sodass $1+ \epsilon$ eine untere Schranke von $S$ ist. Um ein Widerspruch zu erhalten finden wir ein element $x \in S$ sodass $1<x<1+ \epsilon$, was zeigt, dass $1+ \epsilon$ keine untere Schranke von $S$ sein kann. 
\\\\
...
\section{Folgen}
\begin{ex}
Seien $m\in \NN$, $(x_i)_{i\in \NN} \subseteq \RR^m$ und $x\in \RR^m$.
\begin{enumerate}[i.)]
\item
...
Seien $(x_n)_{n\in \NN}$ und $x\in R$ wie in der Aufgabenstellung. Wir nehmen an, dass $(x_n)_{n\in \NN}$ nicht gegen $x$ konvergiert, d.h.
...
\end{enumerate}
\end{solution}
\begin{ex}{$(\star)$} Sei $(z_n)_{n\in \NN} \subseteq \CC$, $z\in \CC$. Entscheiden Sie f\"ur jede Aussage, ob sie wahr ist oder falsch.


\begin{enumerate}[i.)]
\item
$z_n \rightarrow z \Leftrightarrow Re(z_n) \rightarrow Re(z)$ und $Im(z_n) \rightarrow Im(z)$.

\item
$\vert z_n \vert \rightarrow \vert z \vert \Rightarrow z_n \rightarrow z$.

\item
$z_n \rightarrow z \Rightarrow \vert z_n \vert \rightarrow \vert z \vert$.

\item
$z_n \in \RR$ f\"ur alle $n\in \NN$ und $z_n \rightarrow z$ $\Rightarrow$ $z\in \RR$. 
...
\item
Wahr, folgt aus der umgekehrten Dreiecksungleichung und dem Sandwichsatz (Prop 3.8) 
$0\leq \big\vert \ \vert z_n \vert - \vert z \vert \ \big\vert \leq \vert z_n - z \vert \rightarrow 0$.
\item
...
\begin{enumerate}[i.)]
\item
$(\Vert x_n \Vert)_{n \in \NN}$ konvergiert $\Rightarrow$ $(x_n)_{n\in \NN}$ konvergiert.

\item
$(x_n\cdot y_n)_{n\in \NN}$ konvergiert $\Rightarrow$  $(x_n)_{n\in \NN}$ konvergiert.

\item
$(\lambda x_n)_{n\in \NN}$ konvergiert $\Rightarrow$  $(x_n)_{n\in \NN}$ konvergiert.
...
\begin{enumerate}[i.)]
\item
$$ \limsup_{n \rightarrow \infty} (\sqrt{n^2+n}-n)$$

\item
$$\lim_{n \rightarrow \infty} \big( 1 - \frac{5}{n-3} \big)^{(n+\sqrt{n})/2}$$
...
Also insbesondere
$$\limsup_{n \rightarrow \infty} (\sqrt{n^2+n}-n)=\frac{1}{2}.$$
\item
Wir schreiben
$$ \big( 1 - \frac{5}{n-3} \big)^{(n+\sqrt{n})/2} 
= \underbrace{\sqrt{\big( 1 - \frac{5}{n-3} \big)^{n-3}}}_{\rightarrow \sqrt{e^{-5}}=e^{-5/2}} \cdot \underbrace{\sqrt{\big( 1 - \frac{5}{n-3} \big)^{3}}}_{\rightarrow \sqrt{1^3}=1} \cdot \sqrt{\big( 1 - \frac{5}{n-3} \big)^{\sqrt{n}}} .$$

\begin{claim}
$$\lim_{n \rightarrow \infty} \big( 1 - \frac{5}{n-3} \big)^{\sqrt{n}}=1.$$
\end{claim}
\begin{proof}
$$ \log\bigg((1- \frac{5}{n-3})^{\sqrt{n}}\bigg) = \frac{\sqrt{n}}{n-3} \log\bigg((1- \frac{5}{n-3})^{n-3}\bigg) \rightarrow 0 \cdot \log(e^{-5})=0.$$
Damit erhalten wir
$$ \big( 1 - \frac{5}{n-3} \big)^{\sqrt{n}} = e^{\log\big((1- \frac{5}{n-3})^{\sqrt{n}}\big)} 
\rightarrow e^0=1. $$
\end{proof}
...
\rightarrow -\frac{5}{2}, \quad \text{ f\"ur } n \rightarrow \infty.$$
\end{proof}
...
\item
$h$ ist der einzige H\"aufungspunkt in $\RR$ $\Rightarrow$ $a_n \rightarrow h$.

\item
$(a_n)_{n\in \NN}$ konvergiert $\Rightarrow$ $a_n \rightarrow h$.
...
$$ \bigg( 1 +\frac{3}{n} \bigg)^{n+1} = \bigg( 1 +\frac{3}{n} \bigg)^{n} \bigg( 1 +\frac{3}{n} \bigg) \rightarrow e^3 \cdot 1 = e^3, \quad \text{ f\"ur } n \rightarrow \infty.$$

F\"ur $k\in \NN$ und $j\in \{ 1, 3 \}$ gilt
$$ \sin\bigg(\frac{2k \pi}{2}\bigg) = 0 \text{ und } \sin\bigg(\frac{(4k + j)\pi}{2}\bigg)= (-1)^{j+1}.$$
...
\leq \bigg(n \cdot \max_{i\in \{1,\dots, p \}} a_i^n \bigg)^{1/n}$$

$$= n^{1/n} \max_{i\in \{1,\dots, p \}} a_i \rightarrow \max_{i\in \{1,\dots, p \}} a_i, \quad \text{ f\"ur } n \rightarrow \infty.$$
...
\stackrel{a_k>0}{\Leftrightarrow} a_k \leq \sqrt{a_k} + \frac{15}{4} .
\end{align} 
...
$$ (1) \Leftrightarrow x+\frac{15}{4} \geq x^2.$$
...
$$ x^2= x + \frac{15}{4} \Rightarrow x^2 -x - \frac{15}{4} = 0 
\stackrel{x= \sqrt{a}\geq 0}{\Rightarrow} x = \frac{1+\sqrt{1-4\cdot (-\frac{15}{4})}}{2}= \frac{5}{2}.$$
...
= \bigg(1-\frac{1}{n+1} \bigg)^{n+1} \cdot \frac{1}{1-\frac{1}{n+1}} \rightarrow e^{-1}\cdot \frac{1}{1}= e^{-1}, \quad \text{ f\"ur } n \rightarrow \infty.$$
...
Da $\vert \xi_x \vert\leq \vert x \vert$, gilt $\xi_x \rightarrow 0$ f\"ur $x \rightarrow 0$. Damit erhalten wir mit der Stetigkeit der Exponentialfunktion
$$ \lim_{x \rightarrow 0} \bigg( \frac{1}{e^x-1} - \frac{1}{x} \bigg)
...
\item
Wir berechnen
$$ (n^4 - n^2 + 2)^{1/n} = (n^{1/n})^4 \cdot (1 - \frac{1}{n^2} + \frac{2}{n^4})^{1/n} 
\rightarrow 1^4 \cdot 1 = 1, \quad \text{ f\"ur } n\rightarrow \infty$$
und
$$\frac{n + e}{2n + 1} = \frac{n}{2n+1} + \frac{e}{2n+1} = \frac{1}{2+\frac{1}{n}} + \frac{e}{2n+1} \rightarrow \frac{1}{2}, \quad \text{ f\"ur } n\rightarrow \infty.$$
...
$$z_{2k} \rightarrow \frac{i}{2}, \quad \text{ f\"ur } k \rightarrow \infty,$$

$$z_{4k+1} \rightarrow 1-\frac{i}{2}, \quad \text{ f\"ur } k \rightarrow \infty,$$

$$z_{4k+3} \rightarrow -1-\frac{i}{2}, \quad \text{ f\"ur } k \rightarrow \infty,$$
...
\leq  \frac{1}{n+1} \rightarrow 0, \quad \text{ f\"ur } n\rightarrow \infty.$$
Damit gilt
...
\begin{align*}
a_n  &= a_1 + \sum_{k=1}^n (a_{k+1}-a_k) = a_1 + \sum_{k=1}^n (-1)^{k-1} \frac{c}{2^{k-1}} \\
& = a_1 + c\sum_{k=0}^{n-1} \left( \frac{-1}{2} \right)^k 
\end{align*}
Lassen wir nun $n \to \infty$ streben und verwenden die Geometrische Reihe mit $r= -1/2$ erhalten wir 
\begin{align*}
\lim_{n \to \infty} a_n = a_1 + c \frac{1}{1-(-1/2)} = a_1 + c \frac{2}{3} = a_1 + \frac{2}{3}(a_2-a_1) = \frac{a_1-2a_2}{3}
\end{align*}
\end{enumerate}
...
\begin{solution} Dank dem Cauchy'schen Kondensationssatz wissen wir, dass für eine nicht-negative, nicht-wachsende Folge $f(n)$ von reellen Zahl gilt $\sum f(n) < \infty \iff \sum 2^n f(2^n)$ also erhalten wir
\begin{enumerate}[i)]
\item 
\begin{align*}
...
Da $\frac{1}{\log(2k+1)}>0$ und $\lim_{k\rightarrow \infty} \frac{1}{\log(2k+1)}=0$, folgt aus dem Leibnizkriterium (Prop. 4.15), dass $\sum_{k\geq 2} \frac{\sin(k\pi/2)}{\log(k)}$ konvergiert.
...
\end{claim}
\begin{proof}
Wir definieren $h(x)= 2j+1-\log(2j+1)$. 

Dann gilt $h(0)=1$ und $h'(x))= 2 - \frac{1}{2j+1} >0$.
\end{proof}
Damit erhalten wir
...
> \sum_{k\geq 1} \frac{1}{2k+1} > \sum_{k\geq 1} \frac{1}{k}.$$
...
$$ = 2 \bigg(1-\frac{1}{k+1}\bigg)^{k+1} \frac{1}{1-\frac{1}{k+1}} \rightarrow 2 e^{-1} = \frac{2}{e}, \quad \text{ f\"ur } k \rightarrow \infty.$$
...
\item
$$ \sum_{n\geq 0} \frac{\exp(3n \log(n))}{(k!)^2}.$$
\end{enumerate}
\end{ex}
\begin{solution}
\begin{enumerate}[i.)]
...
$$ \frac{\sqrt{n+2}}{n+2} < \frac{\sqrt{n}}{n+1} 
\Leftrightarrow \frac{n+1}{(n+2)^2} < \frac{n}{(n+1)^2}$$

$$\Leftrightarrow n^3+3n^2+3n+1= (n+1)^3 < (n+2)^2 n = n^3+4n^2+4n
\Leftrightarrow 0 < n^2+n-1.$$
...
= \limsup_{n\rightarrow \infty} \bigg( \frac{1+\frac{5}{n}}{3-\frac{2}{n}} \bigg)^{3} = \limsup_{n\rightarrow \infty} \bigg(\frac{1}{3} \bigg)^3= \frac{1}{27} <1.$$
...
Damit ist $(\frac{e^{3n \log(n)}}{(n!)^2})_{n \in \NN}$ keine Nullfolge und mit dem Nullfolgenkriterium (Prop. 4.2) folgt, dass $\sum_{n\geq 0} \frac{\exp(3n \log(n))}{(k!)^2}$ divergiert.
...
$$=\underbrace{\bigg(\Big( 1+ \frac{1}{n} \Big)^{n}\bigg)^3}_{\rightarrow e^3, \text{ f\"ur } n\rightarrow \infty} \cdot \underbrace{(n+1)}_{\rightarrow \infty, \text{ f\"ur } n \rightarrow \infty} \rightarrow \infty.$$
Aus dem Qutientenkriterium (Satz 4.12) folgt, dass $\sum_{n\geq 0} \frac{\exp(3n \log(n))}{(k!)^2}$ nicht absolut konvergiert. Da alle Terme nichtnegativ sind (dann ist absolute Konvergenz und Konvergenz das gleiche), folgt, dass $\sum_{n\geq 0} \frac{\exp(3n \log(n))}{(k!)^2}$ nicht konvergiert.
\end{enumerate}
\end{solution}
...
\end{align*}
Jedoch konvergiert die letzte Reihe nicht, da $\lim \frac{2^n }{n^s} = \infty$ keine Nullfolge ist für alle $s >0$. Also existiert kein $s>0$ sodass die Reihe konvergiert. 
\item
\begin{align*}
\sum_{n=2}^\infty \frac{1}{n \log^s(n)}< \infty &\iff \sum_{n=2}^\infty 2^n \frac{1}{2^n\log^s(2^n)}< \infty \\
& \iff C\sum_{n=2}^\infty \frac{1}{n^s}< \infty 
\end{align*}
...
\begin{align*}
\sum_{k=1}^n \frac{2k^2 +2k+3}{6k^5+6} = \frac{1}{3} \sum_{k=1}^n \frac{k^2}{k^5+1} + \frac{1}{3} \sum_{k=1}^\infty \frac{k}{k^5+1} + \frac{1}{2} \sum_{k=1}^n \frac{1}{k^5 +1}  
\end{align*}
...
\end{align*}
Lassen wir nun $n \to \infty$, dann erkennen wir die konvergenten Reihen,  wir schliessen dass die Reihe konvergiert. 
\\\\
Alternative können wir auch das Vergleichskriterium verwenden, denn es gilt 
\begin{align*}
\sum_{n=1}^\infty \frac{2n^2 + 2n +3}{6n^5 +6} \sim \sum_{n=1}^\infty \frac{1}{n^3}
\end{align*}
Wir berechnen hierfür 
\begin{align*}
\lim_{n \to \infty} \frac{a_n}{b_n} = \lim_{n \to \infty} \frac{\frac{2n^2 + 2n +3}{6n^5 + 6}}{\frac{1}{n^3}} = \lim_{ n \to \infty} \frac{n^3(2n^2+2n+3)}{6n^5+6}= \lim_{n \to \infty} \frac{2n^5}{6n^5}= \frac{1}{3} \in (0, \infty)
\end{align*}
...
\begin{ex} Zeigen Sie, dass die folgende Reihe konvergiert 
\begin{align*}
...
\begin{solution} Wir verwenden das Integralkriterium, d.h. wir betrachten den Integral
\begin{align*}
\int_0^\infty \frac{1}{x^2 +1}dx = \left. \tan^{-1}\right|_{x=0}^\infty = \frac{\pi}{2} < \infty   
\end{align*}
...
\begin{align*}
\lim_{x \to 1} h(x) = \lim_{x \to 1} \frac{x^2 + x+1}{x+1} = \frac{3}{2}
\end{align*}
\end{solution}


\begin{ex}
$f:\RR \rightarrow \RR$ stetig und injektiv, dann ist $f$ streng monoton.
\end{ex}
\begin{solution}
...
Ohne Beschr\"ankung der Allgemeinheit k\"onnen wir annhemen, dass gilt $f(x_1) \leq f(x_2) \geq f(x_3)$. Falls dies nicht der Fall ist, betrachten wir $g: \RR \rightarrow \RR, g(x):=\ -f(x)$. $g$ ist stetig und injektiv, desweiteren ist $g$ genau dann streng monoton, wenn $f$ es ist.

Desweiteren k\"onnen wir ohne Beschr\"ankung der Allgemeinheit annehmen, dass $f(x_1) \leq f(x_3)$. Andernfalls betrachten wir $h: \RR \rightarrow \RR,\ h(x):=f(-x)$.

Wir haben nun alles auf den Fall $f(x_1)\leq f(x_3) \leq f(x_2)$ reduziert. Aus dem Zwischenwertsatz (Satz 6.16) folgt, dass es ein $\xi \in (x_1, x_2)$, sodass $f(x_3)= f(\xi)$, dies steht im Widerspruch, dass $f$ injektiv ist.
\end{solution} 



\begin{ex}{$(\star)$}
Sei $f_n:[0,1] \rightarrow \RR$ eine Funktionenfolge und $f:[0,1] \rightarrow \RR$. Entscheiden Sie f\"ur jede der folgenden Aussagen, ob sie wahr oder falsch ist.
\begin{enumerate}[i.)]
\item
Alle $f_n$ sind stetig und $f_n \rightarrow f$ punktweise $\Rightarrow$ $f$ ist stetig. 
\item
Alle $f_n$ sind stetig und $f_n \rightarrow f$ gleichm\"assig $\Rightarrow$ $f$ ist stetig. 
\item
Alle $f_n$ sind differenzierbar auf $(0,1)$ und $f_n \rightarrow f$ gleichm\"assig $\Rightarrow$ $f$ ist differenzierbar auf $(0,1)$. 
\end{enumerate}
\end{ex}
\begin{solution}
\begin{enumerate}[i.)]
...
\begin{ex}
Sei $f:\RR \rightarrow \RR$ stetig und beschr\"ankt, dann besitzt $f$ einen Fixpunkt.
\end{ex}
\begin{solution}
Wir definieren $a:=\inf_{x\in \RR} f(x)$ und $b:=\sup_{x\in \RR} f(x)$. Dann gilt $Im(f) \subseteq [a,b]$ und wir k\"onnen $f$ einschr\"anken auf $f:[a,b] \rightarrow [a,b]$. Da $f$ stetig ist, folgt die Behauptung aus \"Ubungsblatt 9, Aufgabe 2a.
\end{solution}


\begin{ex}{$(\star)$}
Sei $f:(0,1] \rightarrow \RR$ gleichm\"assig stetig.
Zeigen Sie: Es existiert ein $c\in \RR$, sodass f\"ur jede Folge $(x_n)_{n\in \NN}$ in $(0,1]$ mit $x_n \rightarrow 0$ gilt:
$$ f(x_n) \rightarrow c  \quad \text{ f\"ur } n \rightarrow \infty.$$
Bemerkung (nicht Teil der Aufgabe): Das bedeutet, dass $\lim_{x \downarrow 0}f(x) = c$.
\end{ex}
\begin{solution}
Sei $(x_n)_{n \in \NN}\subseteq (0,1]$ mit $x_n \rightarrow 0$. Dann ist $(x_n)_{n \in \NN}$ eine Cauchyfolge (da sie konvergiert). Da $f$ gleichm\"assig stetig ist, ist $(f(x_n))_{n\in \NN}$ ebenfalls eine Cauchyfolge (Aufgabe 24). Da $\RR$ vollst\"andig ist (Satz 3.23), existiert ein $c\in \RR$, sodass $f(x_n) \rightarrow c$. Sei nun $(y_n)_{n\in \NN}\subseteq (0,1]$, sodass $y_n \rightarrow 0$. Wir definieren f\"ur $n\in \NN$
$$z_{2n}:=x_n \quad \text{ und } \quad z_{2n+1}:= y_n.$$
Dann gilt $z_n \rightarrow 0$. Mit der gleichen Argumentation wie oben, erhalten wir, dass $(f(z_n))_{n\in \NN}$ konvergiert und damit gilt
...
\rightarrow \vert a \vert \cdot 0= 0, \quad \text{ f\"ur } j\rightarrow \infty.$$
...
$$0 \leq e^5  \bigg(\frac{e}{x}\bigg)^{x-5} \leq \bigg(\frac{1}{2}\bigg)^{x-5} \rightarrow 0,
\quad \text{ f\"ur } x\rightarrow \infty.$$

Wobei wir benutzt haben, dass $2^{x-5} \rightarrow \infty$ f\"ur $x\rightarrow \infty$. Damit gilt $\lim_{x\rightarrow \infty} \frac{e^x x^5}{x^x}= 0$.
...
= \frac{9}{1 + \frac{3}{x^2} + \sqrt{1 + \frac{6}{x^2}}} \rightarrow \frac{9}{2}, \quad \text{ f\"ur } x \rightarrow \infty.$$
...
\end{align*}
\item \begin{align*}
\lim_{x \to \infty} \frac{2- \cos x}{x+3}
\end{align*}
\item \begin{align*}
\lim_{x \to \infty} \frac{x^2 (2+ \sin^2x)}{x+100}
\end{align*}
\item \begin{align*}
\lim_{x \to 0 } x^2 e^{ \sin^3 \left( \frac{1}{x}\right) } 
\end{align*}
\end{enumerate}
\end{ex}
...
\begin{enumerate}[i)]
\item \begin{align*}
0 \leftarrow -x^2 \leq x^2 \sin \left( \frac{1}{x} \right) \leq x^2 \to 0, \text{ als } x \to 0 
\end{align*}
da $x^2 \geq 0$.
\item Wir haben $-1 \leq \cos (x) \leq 1$ und somit $ 1 \geq - \cos(x) \geq -1 $ also
\begin{align*}
0 \leftarrow \frac{1}{x+3} \leq \frac{2- \cos x}{x+3} \leq \frac{3}{x + 3} \to 0  \text{ als } x \to \infty 
\end{align*}
...
\begin{align*}
0 \leftarrow x^2 e^{-1} \leq x^2 \exp \left(  \sin^3 \left( \frac{1}{x} \right) \right) \leq x^2 e^1 \to 0, \text{ als } x \to 0 
\end{align*}
\end{enumerate}
\end{solution}
...
\begin{enumerate}[i)]
\item \begin{align*}
 \lim_{x \to 0^+} \left( \sqrt{x^2 +1} \right)^{ \frac{1}{\sin^2 x}} 
\end{align*}
\item \begin{align*}
\lim_{x \to 0^+} x^{ \sqrt{x+4}-2}
\end{align*}
\end{enumerate}
...
\item 
\begin{align*}
 \lim_{x \to 0^+} \left( \sqrt{x^2 +1} \right)^{ \frac{1}{\sin^2 x}} = \lim_{ x \to 0^+} \exp \left( \log \left( ( \sqrt{x^2+1})^\frac{1}{\sin^2x} \right) \right) = \lim_{x \to 0^+} \exp \left( \frac{\log \sqrt{x^2+1}}{\sin^2 x} \right) 
\end{align*}
...
\begin{align*}
\lim_{x \to 0+} \frac{\log ( \sqrt{x^2+1}}{\sin^2x} = \lim_{x \to 0^+} \frac{\log(x^2+1)}{2 \sin^2 x} = \lim_{x \to 0^+} \frac{\frac{2x}{1+x^2}}{4 \sin x \cos x} &= \lim_{x \to 0^+} \underbrace{\frac{x}{2 \sin x}}_{ \to 1/2} \cdot \underbrace{\frac{1}{(1+x^2) \cos x}}_{ \to 1}   
\\ & = \frac{1}{2}
\end{align*}
...
\item 
\begin{align*}
\lim_{x \to 0^+} x^{ \sqrt{x+4}-2} = \lim_{x \to 0^+} \exp \left( \log \left( x^{ \sqrt{x+4}-2} \right) \right) = \lim_{x \to 0^+} \exp \left( (\sqrt{x+4}-2) \log(x) \right) 
\end{align*}
Dann berechnen wir den Grenzwert für die Funktion im Exponenten, erneut mit HIlfe des Satzes von Bernoulli- de L'Hopital:
\begin{align*}
\lim_{ x \to 0^+} ( \sqrt{4+x}-2) \log(x) & = \lim_{x \to 0^+} \frac{\log (x)}{\frac{1}{\sqrt{x+4}-2}} = \lim_{x \to 0^+} \frac{\frac{1}{x}}{\frac{- \frac{1}{2 \sqrt{x+4}}}{(\sqrt{x+4}-2)^2}} \\
&= \lim_{x \to 0^+} \frac{-2 \cdot \sqrt{x+4} \cdot ( \sqrt{x+4}-2)^2}{x} \\
&=  \lim_{x \to 0^+} \frac{-4 ( \sqrt{x+4}-2)^2}{x} = \lim_{x \to 0^+} \frac{-8( \sqrt{x+4}-2) \frac{1}{2 \sqrt{x+4}}}{1} \\
= & \lim_{x \to 0^+} -8 \underbrace{( \sqrt{x+4}-2)}_{ \to 0} \cdot \underbrace{\frac{1}{2 \sqrt{x+4}}}_{ \to 1/4}  = 0 
\end{align*}
...
\begin{align*}
\lim_{x \to 0} \frac{1}{x} \int_0^{\tan x} \log(2+t)dt
\end{align*}
\begin{solution} Wir haben den Typ $"0/0"$ da $\lim_{x \to 0} \int_0^{ \tan x} \log (2 +t) dt =0$ Dank dem Hauptsatz der Integralrechnung erhalten wir 
\begin{align*}
\lim_{x \to 0} \frac{1}{x} \int_0^{\tan x} \log(2+t)dt = \lim_{x \to 0} \frac{\log(2 + \tan (x)) \cdot \frac{1}{\cos^2 (x)}}{1} = \lim_{x \to 0} \frac{\log(2 + \tan(x))}{\cos^2 (x)}= \log(2)
\end{align*}
\end{solution}
\end{ex}
...
\begin{align*}
\lim_{x \to \infty} \left( \frac{x^2 +2x}{x^2-3x+2}\right)^{ \frac{x^2 +1}{x-3}}
\end{align*}

\end{ex}


\begin{solution} Wir verwenden den Fundamentallimes $\lim_{x \to \infty} \left( 1+\frac{1}{x}\right)^x= e$. Wir berechnen also 
\begin{align*}
\lim_{x \to \infty} \left( \frac{x^2 +2x}{x^2-3x+2}\right)^{ \frac{x^2 +1}{x-3}} &= \lim_{x \to \infty} \left( \frac{x^2-3x+2+3x-2+2x}{x^2-3x+2} \right)^\frac{x^2+1}{x-3} \\
&= \lim_{x \to \infty} \left( 1 + \frac{5x-2}{x^2-3x+2} \right)^\frac{x^2+1}{x-3} \\
&= \lim_{x \to \infty} \left[ \left( 1 + \frac{1}{\frac{x^2-3x+2}{5x-2}}\right)^\frac{x^2-3x+2}{5x-2}\right]^{ \frac{5x-2}{x^2-3x+2} \frac{x^2+1}{x-3}}
\end{align*}
...
\begin{align*}
\frac{5x-2}{x^2-3x+2} \frac{x^2+1}{x-3} =\frac{5x^3-2x^2+5x-2}{x^3-6x^2+12x-6} \to 5 \text{ als } x \to \infty 
\end{align*}
...
\begin{align*}
\lim_{x \to \infty} f(x)^{g(x)} = \lim_{x \to \infty} f(x)^{\lim g(x)} 
\end{align*}
...
\begin{align*}
\lim_{x \to \infty} f(x)^{g(x)} = \lim_{x \to \infty} \exp \left( \log \left( f(x)^{g(x)} \right) \right) = \lim_{x \to \infty} \exp \left( g(x) \log(f(x)) \right) = \exp( \lim g(x) \log ( \lim f(x)))  \\
= \lim f(x)^{\lim g(x)}
\end{align*}
...
\begin{ex}
Sei $f:\RR_{> -1} \rightarrow \RR, \ f(x)=\log(1+x)$.
\begin{enumerate}[i.)]
\item
...
$$ R_2 f(x) \geq 0, \quad \forall \ 0\leq x < R. $$
\item
...
Wir nehmen nun an, dass die Behauptung f\"ur $n\in \NN$ wahr ist. Dann berechnen wir
...
$$ \rho = \frac{1}{\limsup_{n \rightarrow \infty} n^{1/n}} = \frac{1}{1}= 1.$$
\item
...
\stackrel{\text{Behauptung 8}}{=} \frac{2!(-1)^4}{3!(1+\xi_x)^3}x^3
...
\begin{ex}
Es sei $f_n : [1, \infty) \rightarrow \RR, \ f_n(x)= 2n \left( \left(2x \right)^{1/n} -1 \right)$.
\begin{enumerate}[i.)]
\item
Zeigen Sie, dass $(f_n)_{n\in \NN}$ punktweise konvergiert und bestimmen Sie den Grenzwert 
$f:[1, \infty) \rightarrow \RR$.
...
$$= 2\cdot \log(2x) + \frac{e^{\xi_x}}{2n} \log(2x)^2 \rightarrow 2\log(2x)=:f(x), \quad
 \text{ f\"ur } n\rightarrow \infty.$$
 
\item
Aus $i.)$ wissen wir, dass es f\"ur jedes $x\in [1,2]$ ein $\xi_x \in (0, 1/n \log(2x))$, sodass
$$ \vert f_n(x) - f(x) \vert 
...
\leq \frac{2 \cdot \log(16)}{ n} \rightarrow 0, \quad \text{ f\"ur } n \rightarrow \infty.$$
\item
Nein.
 $$\bigg\vert f_n\left(\frac{n^n}{2}\right) - f\left(\frac{n^n}{2}\right) \bigg\vert
 = \vert 2n \left( n - 1 \right) - 2\log(n^n) \vert
 = \vert 2n (n-1 - \log(n)) \vert \rightarrow \infty, \quad \text{ f\"ur } n \rightarrow \infty.$$
 Also 
 $$\sup_{x\in [1, \infty)} \vert f_n(x) - f(x) \vert \rightarrow \infty, \quad \text{ f\"ur } n \rightarrow \infty.$$
\end{enumerate}
\end{solution}
...
\leq n^{1/n} \rightarrow 1, \quad \text{ f\"ur } n \rightarrow \infty.$$
Damit gilt 
...
= \underbrace{4^{1/n}}_{\rightarrow 1} \cdot \underbrace{\left( n^{1/n} \right)^{5/2}}_{\rightarrow 1^{5/2}=1} \cdot \underbrace{e^{\log(3)/\sqrt{n}}}_{\rightarrow e^0=1} \rightarrow 1, \quad \text{ f\"ur } n \rightarrow \infty.$$
Damit erhalten wir
...
$$ \left( n! \right)^{1/n} 
\geq \left(1\cdot 2 \cdot \cdot 3 \cdot \dots (k-1) \cdot k \underbrace{k \cdot \dots \cdot k}_{n-k \text{ mal}} \right)^{1/n}
\geq k^{(n-k)/n} = k^{1- k/n}$$
Damit gilt
...
$p_m(z)= \sum_{n=0}^m a_n z^n \rightarrow f(z)$ gleichm\"assig auf $\{ z \in \CC \ : \ \vert z \vert \leq r  \}$ f\"ur alle $r<\rho$.
\item
Es existiert eine analytische Funktion $f: \RR \rightarrow \RR$ mit $f(x)= 0$ f\"ur alle $\vert x \vert >2$ und $f(x)=1$ f\"ur alle $\vert x \vert \leq 1$.
\end{enumerate}
\end{ex}
\begin{solution}
\begin{enumerate}[i.)]
...
Wahr, Satz 8.30.
\item
Sei $0\leq r < \rho$. Nach Satz 7.2 konvergiert $\sum_{n\geq 0} \vert a_n \vert \cdot r^n$. Insbesondere gilt $\sum_{n\geq m+1} \vert a_n \vert \cdot r^n \rightarrow 0$ f\"ur $m\rightarrow \infty$. Die folgende Rechnung zeigt die gleichm\"assige Konvergenz auf $\{ z \in \CC \ : \ \vert z \vert \leq r  \}$
...
$$= \sum_{n\geq m+1} \vert a_n \vert \cdot r^n \rightarrow 0, \quad \text{ f\"ur } m \rightarrow \infty.$$
\item
Falsch. Siehe n\"achste Aufgabe.
\end{enumerate}
\end{solution}
...
\begin{ex}
Seien $a\in \RR, \epsilon>0,$ und $f:\RR \rightarrow \RR$ analytisch mit $f|_{(a, a+\epsilon)}=0$. Wir wollen zeigen, dass $f=0$.
\begin{enumerate}[i.)]
\item
Zeigen Sie $\{ x\in \RR_{> a} \ :  \ f|_{(a,x)}=0 \} \neq \emptyset. $
\item
Definieren Sie $b:=\sup \{ x\in \RR_{> a} \ :  \ f|_{(a,x)} \}$. Nutzen Sie das Identit\"atsprinzip um (mittels Widerspruchsbeweis) zu zeigen, dass $b=\infty$.
\item
Schliessen Sie, dass $f=0$.
\end{enumerate}
\end{ex}
\begin{solution}
\begin{enumerate}[i.)]
\item
Nach Voraussetzung $\epsilon \in \{ x\in \RR_{> a} \ :  \ f|_{(a,x)}=0 \}$ und damit $\{ x\in \RR_{> a} \ :  \ f|_{(a,x)}=0 \}\neq \emptyset$.
\item
Annahme $b\neq \infty$. Dann ist $f(b)=0$, da $f|_{(a,b)}=0$ und $f$ stetig (da $f$ analytisch, Kor. 8.31). Aus dem Identit\"atsprinzip (Prop. 8.33) folgt, dass $f$ in einer Umgebung von $b$ identisch gleich Null ist. Damit existiert ein $\delta >0$, sodass $f|_{(b-\delta, b+\delta)}=0$. Dann gilt jedoch $f|_{(a, b+\delta)}=0$ Dies ist ein Widerspruch dazu, dass $b=\sup \{ x\in \RR_{> a} \ :  \ f|_{(a,x)} \}$.
\item
Aus $ii.)$ folgt, dass $f|_{(a,\infty)}=0$. Wir m\"ussen nur noch zeigen, dass 
$f|_{(-\infty,a)}=0$. Wir definieren $g(x):=f(-x)$. Da $f$ und $x\mapsto-x$ analytisch sind und die Komposition analytischer Funktionen analytisch ist (siehe Seite 110), folgt, dass $g$ analytisch ist. Es gilt $g|_{(-a-\epsilon, -a)}= f|_{(a, a+\epsilon)}=0$. Aus $ii.)$ folgt, dass $g|_{(-a, \infty)}=0$ und damit $f|_{(-\infty, a)}=0$.
\end{enumerate}
\end{solution}
...
\begin{ex}{$(\star)$}
Sei $f:\RR \rightarrow \RR$ eine gerade (d.h. $f(x)=f(-x)$ f\"ur alle $x\in \RR$), differenzierbare Funktion. Zeigen Sie, dass $f':\RR \rightarrow \RR$ eine ungerade Funktion ist, d.h. $f'(-x)=-f'(x).$
\end{ex}
\begin{solution}
...
\begin{ex}
Beweisen Sie die verallgemeinerte Bernoulli-Ungleichung, diese besagt, dass f\"ur alle $r\geq 1, \ x\geq$ gilt:
$$ (1+x)^r\geq 1+rx .$$
...
\begin{ex}{$(\star)$}
Sei $f:\RR \rightarrow \RR$ definiert durch
...
\leq \vert h \vert \rightarrow 0, \quad \text{ f\"ur } h\rightarrow 0.$$
...
$$ \cos\left(\frac{1}{\bigg( \frac{1}{2n\pi} \bigg)}\right) = \cos(2n\pi) = 1 \quad \text{ und }  \frac{1}{2n\pi} \rightarrow 0 \quad \text{ f\"ur } n \rightarrow \infty.$$
...
\begin{ex}
Sei $\epsilon>0$. Wir definieren $f_{\epsilon}:\RR \rightarrow \RR, f_{\epsilon}(x)= \vert x \vert^{1+\epsilon}$.
Zeigen Sie, dass $f_{\epsilon}$ auf ganz $\RR$ differenzierbar ist.
\end{ex}
\begin{solution}
Wir wissen, dass der Absolutbetrag ausserhalb von $x=0$ differenzierbar ist. Auch wissen wir, dass $x\mapsto x^{1+\epsilon}$ differenzierbar ist (Seite 90). Die Komposition differenzierbarer Funktionen ist differenzierbar (Kettenregel, Prop. 8.5), also ist $f_{\epsilon}$ in $x\neq 0$ differenzierbar. Nun zeigen wir noch, dass $f_{\epsilon}$ in $x=0$ differenzierbar ist

$$\bigg\vert \frac{f_{\epsilon}(h)- f_{\epsilon}(0)}{h-0} \bigg\vert 
= \bigg\vert \frac{\vert h \vert^{1+\epsilon}}{h} \bigg\vert 
= \vert h \vert^{\epsilon} \rightarrow 0, \quad h \rightarrow 0.$$
...
$$f:\RR \rightarrow \RR, f(x)= \left\{\begin{array}{lr}  x^2 , & x\in \QQ \\
...
$$ \bigg\vert \frac{f(h)-f(0)}{h-0} \bigg\vert \leq \bigg\vert \frac{h^2}{h} \bigg\vert = \vert h \vert \rightarrow 0,
\quad h\rightarrow 0.$$
...
$$a-b t >0 \Leftrightarrow t > -\frac{a}{b} \quad \text{ und }
\quad a+b t >0 \Leftrightarrow t < \frac{a}{b}.$$
Damit gilt
$$\frac{a+b t}{a-b t} \geq 0 \Leftrightarrow t\in \left( -\frac{a}{b}, \frac{a}{b} \right].$$
Wir rechnen
...
\begin{ex}{$(\star)$}
Seien $f,g : (a,b) \rightarrow \RR$ differenzierbare Funktionen.
\begin{enumerate}[i.)]
\item
...
Da $f,g$ stetig sind, ist $f-g$ ebenfalls stetig (Prop. 6.3) und damit existiert ein $\delta>0$, sodass gilt $(f-g)|_{(x-\epsilon, x+\epsilon)}>0$. Damit gilt
...
$"\Leftarrow"$ Sei nun $f'(x)=g'(x)$ f\"ur ein fixes $x\in (a,b)$. Sei $\epsilon>0$.
...
< \epsilon.$$
Damit gilt $\lim_{h\in \rightarrow 0} \frac{\max(f,g)(x+h) - \max(f,g)(x)}{h}$ konvergiert (gegen $f'(x)$), also ist $\max(f,g)$ in $x$ differenzierbar.
\end{proof}
\end{enumerate}
\end{solution}
...
Die Induktionsverankerung $n=1$ folgt direkt aus dem Mittelwertsatz (Satz 8.9). Wir nehmen an, die Aussage gilt f\"ur $n\in \NN_{\geq 1}$. Seien jetzt $0\leq x_1 < \dots < x_{n+2}\leq 1$.
...
Man beachte, dass $(C^1(\RR, \RR), \Vert \cdot \Vert_{C^0(\RR, \RR)})$ kein normierter Vektorraum ist und darum die urspr\"ungliche Aufgabe keinen Sinn macht (zum Beispiel ist $id \in C^1(\RR, \RR)$, aber $\sup_{x\in \RR} \vert id(x) \vert = \sup_{x\in \RR} \vert x \vert =\infty$). Weiter waren die urspr\"unglichen Funktionen nicht einmal stetig und insbesondere also nicht differenzierbar. Damit war auch das Gegenbeispiel zu Aufgabe 30 falsch. Man kann die obigen Funktionen benutzen um ein echtes Gegenbeispiel zu konstruieren.
\begin{enumerate}[i.)]
\item
...
$$ f: \RR \rightarrow \RR, 
f(x)= \ \vert x \vert. $$
Wir zeigen, dass $f_n \rightarrow f$ bez\"uglich $\Vert \cdot \Vert_{C^0([-K, K], \RR)}$. F\"ur $n\geq 1/K$ gilt
...
= \frac{2}{n} \rightarrow 0, \quad \text{ f\"ur } n \rightarrow \infty.$$
\item
...
Zuerst zeigen wir $x=0$ ein lokales Minimum von $f$ ist. Dazu bemerken wir $f(0)=0$. Weiter gilt
$$ 0<h \Rightarrow f(h) = \underbrace{(1+h)}_{>0} \underbrace{\sqrt{h}}_{>0} >0$$
und
$$ -1<h<0 \Rightarrow f(h) = \underbrace{(1+h)}_{>0} \underbrace{\sqrt{-h}}_{>0} >0.$$
...
\Leftrightarrow 0=  3x + 1 \Leftrightarrow x= -\frac{1}{3}.$$
...
$$ 1= e^0 \leq \frac{e^x - e^0}{x-0} = \frac{e^x-1}{x} \Rightarrow 1+x \leq e^x.$$
Falls $x<0$, setzen wir $z=x, y=0$
$$ \frac{1-e^x}{-x} = \frac{e^0 -e^x}{-x} \leq e^0 =1  \Rightarrow 1-e^x \leq -x 
...
Sei $n\in \NN_{\geq 1}$ und $f: \RR_{> 0} \rightarrow \RR, f(x)=x^n$. Es gilt $f^{(2)}(x)= n(n-1)x^{n-2}\geq 0$. Mit Prop. 8.21 ist $f$ konvex und die Aussage folgt aus Prop. 8.20.
\end{solution}
...
\begin{enumerate}[i.)]
\item
Als Folge in $C^0([0,1], \RR)$ hat $f_n(x)=x^n$ keine konvergente Teilfolge.
\item
Die Funktion $f:\RR \rightarrow \RR$ definiert durch $f(x)=x^2 \cos(1/x)$ f\"ur $x\neq0$ und $f(0)=0$ ist in $x=0$ differenzierbar 
\item
Sei $f:\RR \rightarrow \RR$ konvex. Dann ist $f$ auf $\RR$ differenzierbar.
\item
Ist $f:[0,1] \rightarrow \RR$ gleichm\"assig stetig und auf $(0,1)$ differenzierbar, dann ist $f'$ beschr\"ankt.
\item
Ist $f:[0,1] \rightarrow \RR$ differenzierbar und es gibt ein $x_0\in (0,1)$ mit $f'(x_0)=0$, so hat $f$ in $x_0$ ein lokales Extremum.
\end{enumerate} 
\end{ex}
\begin{solution}
\begin{enumerate}[i.)]
\item
Wahr. Nehmen wir an, es g\"abe eine gleichm\"assig konvergente Teilfolge $(f_{n_k})_{k\in \NN}$. Diese konvergiert punktweise gegen $f:[0,1]\rightarrow \RR, f(x) = \begin{cases} 0, \quad x\neq 1 \\ 1, \quad x=1 \end{cases}$. Damit $f_{n_k} \rightarrow f$ gleichm\"assig. Da die $f_n$ stetig sind, ist $f$ stetig (Prop. 6.24). Dies ist offensichtlich nicht der Fall, also Widerspruch.
\item
Wahr. Es gilt
$$ 0\leq \bigg\vert \frac{h^2 \cos(\frac{1}{h}) - 0}{h-0} \bigg\vert = \vert h \vert \cdot \big\vert \cos\big(\frac{1}{h}\big) \big\vert \leq \vert h \vert \rightarrow 0, \qquad \text{f\"ur } h\rightarrow 0.$$

Damit gilt $\lim_{h\rightarrow 0} \frac{f(h)- f(0)}{h-0}=0$. Also ist $f$ in $x=0$ differenzierbar (mit $f'(0)=0$).
\item
Falsch, der Absolutbetrag ist ein Gegenbeispiel (nicht differenzierbar in $x=0$).
\item
Falsch. Gegenbeispiel $f:[0,1] \rightarrow \RR, f(x) = \begin{cases} x^2 \cos(\frac{1}{x^3}), \qquad x\neq 0 \\ 0, \qquad \qquad \qquad x=0.\end{cases}$ Man beweist analog zu $ii.)$, dass $f$ stetig ist. Da $[0,1]$ kompakt ist, ist $f$ gleichm\"assig stetig (Satz 6.22). Mittels der Kettenregel (Prop. 8.5) erhalten wir f\"ur $x\in (0,1):$ $f'(x)= 2x \cos(\frac{1}{x^3})+ \frac{1}{x^2} \sin(\frac{1}{x^3})$. Der erste Term ist durch 1 beschr\"ankt und der zweite ist unbeschr\"ankt, also ist $f'$ unbeschr\"ankt.

Ein alternatives Gegenbeispiel ist $g:[0,1]\rightarrow \RR, \ g(x)=\sqrt{x}$. Wieder folgt aus Satz 6.22, dass $g$ gleichm\"assig stetig ist. Jedoch ist $g'(x)= 1/\sqrt{x}$ unbeschr\"ankt auf $(0,1)$.
\item
...
\begin{enumerate}[i)]
\item Es sei $f: [a,b] \to [0, \infty)$. Dann gilt $\displaystyle \int_a^bf(x)dx = 0 \iff f \equiv 0$ 
\item Seien $f,g : [a,b] \to \mathbb{R}$ sodass $f \leq g$. Dann gilt $\displaystyle \int_a^b f(x)dx \leq \int_a^b f(x) dx$.
\item Sei $f: [a,b] \to \mathbb{R}$. Dann impliziert $\displaystyle \int_a^b f(x) dx >0$ dass $f>0$. 
\item Sei $f:[0, \infty) \to \mathbb{R}_+$ monoton fallend mit $\displaystyle \int_0^\infty f(x) dx < \infty$. Dann gilt $\displaystyle \sum_{n=0}^\infty f(n) < \infty$. 
...
\item
Falsch. Der negative Teil von $f$ muss nur eine kleinere Fläche haben, als der positive Teil, z.B. $f:[-1,3] \rightarrow \RR, ~ x \mapsto x$. Dann gilt
...
\begin{ex} Es sei $t \in \mathbb{R}_1$. Berechnen Sie die folgenden Integrale. 
\begin{enumerate}[i)]
\item \begin{align*}
\int_0^t x^2 ( \ln(x))^2 dx
\end{align*}
\item \begin{align*}
...
\end{align*}
\item \begin{align*}
\int_2^4 \frac{x^4 +x^3 +x^2+1}{x^2+x-2}
\end{align*}
\end{enumerate}
...
&= \bigg( -\frac{1}{2\sqrt{x+1}} + \frac{1}{2\sqrt{x+2}} \bigg) \Bigg|^1_0\\
&= -\frac{1}{2\sqrt{2}} + \frac{1}{2\sqrt{3}} +  \frac{1}{2} - \frac{1}{2\sqrt{2}}\\[.1cm]
&= -\frac{1}{\sqrt{2}} + \frac{1}{2\sqrt{3}} +  \frac{1}{2}. 
\notag
\end{aligned}
\end{equation}
...
\end{align*}
Nun liefert die Integrations über die Grenzen schliesslich
\begin{align*}
